...
! All parameters should be specifed in atomic units to avoid any
! complications.
...
! What kind of run to perform (CI is conical intersection, SM is polariton Shin-Metinu)
INTEGER, PARAMETER :: run_type = SM
! Should the CI off-diagonal coupling be \lambda*|Q_c| rather than \lambda*Q_c 
INTEGER, PARAMETER :: abs_coupling = 0  ! This is a legacy feature. Do not trust it.

! Time step for rk4 integration routines
...
REAL*8, PARAMETER :: omegas_t = 0.002279d0 ! Coupling coordinate frequency
REAL*8, PARAMETER :: omegas_c = 0.004116d0 ! Tuning coordinate frequency
REAL*8, PARAMETER, DIMENSION(2) :: kappa_t = (/-0.006836d0, 0.006836d0/) ! Shift to well minima
REAL*8, PARAMETER, DIMENSION(2) :: exe = (/-0.00139d0,0.00139d0/) ! Base energies of both wells
...
! These are legacy initialization parameters.
